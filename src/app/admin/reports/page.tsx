    ...
    <section>
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-[var(--muted)]">
          These reports use first-party anonymous visitor hashes. Security fingerprints and raw identifiers are never displayed.
        </p>
      </div>
      <div className="mt-6 divide-y divide-[var(--line)] border-y border-[var(--line)]">
        {reports.length ? reports.map(({ battle, champion, challenger }) => (
          <Link
            href={`/admin/reports/${battle.id}`}
            key={battle.id}
            className="flex flex-col gap-3 py-5 hover:text-[var(--accent)] sm:flex-row sm:items-center"
          >
            <StatusPill tone={battle.status === "live" ? "live" : "neutral"}>{battle.status}</StatusPill>
