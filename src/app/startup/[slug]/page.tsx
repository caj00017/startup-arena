import { ArrowUpRight, Clock3, Flame, Swords, Trophy } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-14 sm:py-20">
      <div className="flex flex-col gap-6 border-b-2 border-[var(--foreground)] pb-10 sm:flex-row sm:items-center">
        ...
        <div className="flex-1"><div className="flex flex-wrap gap-2"><StatusPill tone="good">{data.startup.status}</StatusPill><StatusPill>{data.startup.launchStatus}</StatusPill>{data.isCurrentChampion && <StatusPill tone="live">Current champion</StatusPill>}</div><h1 className="mt-3 font-display text-6xl font-black tracking-[-0.055em]">{data.startup.name}</h1><p className="mt-3 text-lg text-[var(--muted)]">{data.startup.tagline}</p></div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat icon={<Clock3 size={19} />} value={formatCrownTime(data.crownTimeMs)} label="Crown time" />
        <Stat icon={<Flame size={19} />} value={data.longestStreak} label="Best streak" />
      </div>
